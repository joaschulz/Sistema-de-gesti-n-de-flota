...
class UsuarioDAO {
    ...
    public function obtenerPorUsuario($nombreUsuario) {
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, usuario, password, rol, estado FROM usuarios WHERE usuario = :usuario");
    ...
    public function obtenerTodos() {
        try {
            $pdo = Conexion::conectar();
            $stmt = $pdo->query("SELECT id, usuario, rol, estado, ultimo_acceso, fecha_creacion FROM usuarios ORDER BY id ASC");
    ...
    public function crear($usuario, $password, $rol) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO usuarios (usuario, password, rol, estado) VALUES (:usuario, :password, :rol, 'Activo')");
            return $stmt->execute([':usuario' => $usuario, ':password' => $hash, ':rol' => $rol]);
        } catch (PDOException $e) {
            return false;
        }
    }
